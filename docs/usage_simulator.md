...
`MatAssetLoader` コンポーネントで、マットのテクスチャ（画像）を変更することも可能です。（ただし「カスタム」以外のものを変更することはおすすめしません。） 「カスタム」の画像変更は以下のように行います。
1. 画像ファイルをプロジェクトにインポートする
2.  対象の画像の「インスペクター」上で「テクスチャタイプ」を「スプライト（2DとUI）」に設定し、「適用する」ボタンをクリックする
...
- `Trigger DoubleTap`： ダブルタップを発生させるボタンです。（現在、ゲームエンジンで物理的なダブルタップ・シミュレーションは実装されていません）
...
- カメラに `Physics Raycaster` コンポーネントを追加してください；
  - 追加方法：カメラのインスペクターで、「Add Component」で `Physics Raycaster` を見つけて追加します。
