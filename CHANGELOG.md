...
- [CubeManager] Remove method `CubeManager.SetNearestScanner` and `CubeManager.SetNearScanner` and add method `CubeManager.SetCubeScanner`, since to replace filed `nearestScanner` and `nearScanner` with `scanner`.
