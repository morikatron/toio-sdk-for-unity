# Technical Documentation - How to Use - Simulators
...
- [2. Mat Prefab](usage_simulator.md#2-mat-prefab)
  ...
  - [2.4. Method](usage_simulator.md#24-method)
...
Simulator is a virtual environment for testing smart device applications that communicate with the toio™ Core Cube (Cube).
...
### Manually change the state of Cube (at runtime)
...
- `button State`: If you check the box, the button will be held down. If you want to move Cube again, you will have to uncheck the box and release the button. Also, if you change the button state with the mouse, this setting will be overwritten.
- `[Make sloped changes manually]`: If checked, the `sloped status` will be displayed and you will be able to change the settings.
(Normally, Simulator sets the sloped state for each frame, but if this setting is enabled, Simulator will not set the sloped state.
  - `sloped state`: Indicates whether Cube is on a slope.
- `Causing collision`: This is the button that generates collisions. (Currently, physical collision simulation is not implemented in the game engine.
- `Generate a double tap`: This is the button that generates the double-tap. (Currently, physical double-tap simulation is not implemented in the game engine.
- `pose state`: You can display and change the attitude of Cube.
