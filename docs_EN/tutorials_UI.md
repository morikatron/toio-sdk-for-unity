...
The above script implements the following three things.
    1. Since this is a process that we want to execute only in Simulator, we have written the entire implementation of the class in "#if [UNITY_EDITOR]".
    2. The camera has been moved to the left because we want Simulator screen to be slightly to the right.
    3. Set the parent of [UI]-[Canvas] to [SimCanvas], and then move it to the left edge.(Using [Transform.SetParent])
