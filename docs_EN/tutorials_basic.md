...
This SDK allows you to run the toio™ Core Cube (Cube) in both simulator and smart device environments with the same source code.
...
Each event conforms to the toio™ Core Cube Technical Specification, so please refer to that for details.
...
# 10. Midway connection/disconnection
